...
function mapPanRange(){ const bounds=worldBoundsAtPan(0,0); const rangeFor=(start,end,size)=>{ if(end-start<=size) return { min:size*.5-(start+end)*.5, max:size*.5-(start+end)*.5 }; return { min:size-end, max:-start }; }; return { x:rangeFor(bounds.left,bounds.right,innerWidth), y:rangeFor(bounds.top,bounds.bottom,innerHeight) }; }
function backgroundPanRange(){ const zoom=Math.max(state.gridZoom/BASE_GRID_ZOOM,1); const bgW=(innerWidth+4)*zoom, bgH=(innerHeight+4)*zoom; const rangeFor=(bgSize,viewSize)=>{ const half=Math.max(0,(bgSize-viewSize)/2); return { min:-half, max:half }; }; return { x:rangeFor(bgW,innerWidth), y:rangeFor(bgH,innerHeight) }; }
function intersectPanRange(world,bg){ const rangeFor=(a,b)=>{ const min=Math.max(a.min,b.min), max=Math.min(a.max,b.max); return min<=max ? {min,max} : b; }; return { x:rangeFor(world.x,bg.x), y:rangeFor(world.y,bg.y) }; }
function panRange(){ return intersectPanRange(mapPanRange(), backgroundPanRange()); }
...
window.__brickCityQA = {
  ...
  run(){
    ...
    assert('topdown road asset contract includes reusable connection shapes', state.roadTypes.every(t => ROAD_SHAPES.every(shape => t.assets?.variants?.[`topdown-${shape}`])));
